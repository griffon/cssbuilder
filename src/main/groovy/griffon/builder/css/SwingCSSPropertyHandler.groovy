...
class SwingCSSPropertyHandler implements CSSPropertyHandler {
    ...
    private Border calculateMargin(JComponent component, String value) {
        ...
        if (!BorderUtils.isMargin(currentBorder)) {
            ...
            BorderUtils.setBorderInsets(newBorder, BorderUtils.getBorderInsets(margin))
    ...
    private Border calculateMargin(JComponent component, String propertyName, String value) {
        ...
        if (!BorderUtils.isMargin(currentBorder)) {
            ...
            Border newBorder = BorderFactory.createCompoundBorder(margin, currentBorder)
            BorderUtils.setBorderInsets(newBorder, insets)
            BorderUtils.setAsMargin(newBorder)
            currentBorder = newBorder
        } else {
            // currentBorder must be a compoundBorder
            Insets insets = merge(newInsets(propertyName, val, coord), BorderUtils.getBorderInsets(currentBorder), coord)
            Border margin = createEmptyBorder(insets)
            if (!margin) return currentBorder
            Border newBorder = BorderFactory.createCompoundBorder(margin, currentBorder.insideBorder)
            BorderUtils.setBorderInsets(newBorder, insets)
    ...
    private Border calculatePadding(JComponent component, String value) {
        ...
        if (BorderUtils.isPadding(currentBorder)) {
            ...
            BorderUtils.setBorderInsets(newBorder, BorderUtils.getBorderInsets(padding))
        ...
        if (BorderUtils.isMargin(component.border)) {
            ...
            BorderUtils.setBorderInsets(currentBorder, BorderUtils.getBorderInsets(component.border))
    ...
    private Border calculateMatteBorder(Border originalBorder, Color color) {
        ...
        if (BorderUtils.isPadding(currentBorder)) {
            ...
            Insets borderInsets = BorderUtils.getBorderInsets(outer)
            ...
            BorderUtils.setBorderColor(newBorder, color)
            BorderUtils.setBorderInsets(newBorder, insets)
            BorderUtils.setAsPadding(newBorder)
            BorderUtils.clear(currentBorder)
            currentBorder = newBorder
            ...
            // wack it
            Insets borderInsets =  BorderUtils.getBorderInsets(currentBorder)
            Insets insets = new Insets(
                borderInsets?.top != null ? borderInsets.top : 0,
                borderInsets?.left != null ? borderInsets.left : 0,
                borderInsets?.bottom != null ? borderInsets.bottom : 0,
        ...
        if (BorderUtils.isMargin(originalBorder)) {
            ...
            BorderUtils.setBorderInsets(currentBorder, BorderUtils.getBorderInsets(originalBorder))
            BorderUtils.setAsMargin(currentBorder)
        }
        return currentBorder
    }
    ...
    private Border calculateMatteBorder(Border originalBorder, int width) {
        ...
        if (BorderUtils.isPadding(currentBorder)) {
            ...
            Border newBorder = currentBorder ? BorderFactory.createCompoundBorder(createMatteBorder(insets, BorderUtils.getBorderColor(currentBorder)), currentBorder.insideBorder) : createMatteBorder(insets, Color.BLACK)
            BorderUtils.setBorderColor(newBorder, BorderUtils.getBorderColor(outer) ?: Color.BLACK)
            BorderUtils.setBorderInsets(newBorder, insets)
            BorderUtils.setAsPadding(newBorder)
            BorderUtils.clear(currentBorder)
            currentBorder = newBorder
            ...
            // wack it
            Insets insets = new Insets(width, width, width, width)
            Border newBorder = createMatteBorder(insets, BorderUtils.getBorderColor(currentBorder))
            BorderUtils.setBorderColor(newBorder, BorderUtils.getBorderColor(currentBorder) ?: Color.BLACK)
            currentBorder = newBorder
        }
        if (BorderUtils.isMargin(originalBorder)) {
            BorderUtils.clear(originalBorder)
    ...
    private Border calculateMatteBorder(Border originalBorder, String propertyName, int offset, int width) {
        ...
        if (BorderUtils.isPadding(currentBorder)) {
            Border outer = currentBorder?.outsideBorder
            Insets insets = merge(newInsets(propertyName, width, coord),  BorderUtils.getBorderInsets(outer), coord)
            Border newBorder = currentBorder ? BorderFactory.createCompoundBorder(createMatteBorder(insets, BorderUtils.getBorderColor(currentBorder)), currentBorder.insideBorder) : createMatteBorder(insets, Color.BLACK)
            BorderUtils.setBorderInsets(newBorder, insets)
            BorderUtils.setBorderColor(newBorder, BorderUtils.getBorderColor(currentBorder) ?: Color.BLACK)
            BorderUtils.setAsPadding(newBorder)
            ...
            // wack it
            Insets insets = merge(newInsets(propertyName, width, coord), BorderUtils.getBorderInsets(currentBorder), coord)
            Border newBorder = createMatteBorder(insets, BorderUtils.getBorderColor(currentBorder))
            BorderUtils.setBorderColor(newBorder, BorderUtils.getBorderColor(currentBorder) ?: Color.BLACK)
            currentBorder = newBorder
        }
    ...
    private static Dimension calculateSize(Dimension originalSize, String prop, int value) {
    ...
    private static Border createEmptyBorder(Insets insets) {
    ...
    private static Border createMatteBorder(Insets insets, Color color) {
        Border border = BorderFactory.createMatteBorder(insets.top, insets.left, insets.bottom, insets.right, color)
        BorderUtils.setBorderInsets(border, insets)
        return border
    }

    private static Insets newInsets(String propertyName, String[] values) {
        Insets insets = null
        switch (values.size()) {
    ...
    private static Insets newInsets(String propertyName, String size) {
        int val = normalizeSize(propertyName, size)
        if (val == -1) return null
        return new Insets(val, val, val, val)
    }

    private static Insets newInsets(String propertyName, String size, String coord) {
        int val = normalizeSize(propertyName, size)
        if (val == -1) return null
        return newInsets(propertyName, val, coord)
    }

    private static Insets newInsets(String propertyName, int val, String coord) {
        return new Insets(
            coord == "top" ? val : 0,
            coord == "left" ? val : 0,
            coord == "bottom" ? val : 0,
    ...
    private static Insets merge(Insets a, Insets b, String coord) {
        return new Insets(
    ...
    private static int getAutoResizeMode(String propertyName, String propertyValue) {
    ...
    private static int getSelectionMode(String propertyName, String propertyValue) {
        switch (propertyValue.toUpperCase()) {
    ...
    private static int getTabPlacement(String propertyName, String propertyValue) {
        switch (propertyValue.toUpperCase()) {
    ...
    private static int getTabLayoutPolicy(String propertyName, String propertyValue) {
        switch (propertyValue.toUpperCase()) {
    ...
    private static int getHorizontalScrollBarPolicy(String propertyName, String propertyValue) {
        switch (propertyValue.toUpperCase()) {
    ...
    private static int getVerticalScrollBarPolicy(String propertyName, String propertyValue) {
        switch (propertyValue.toUpperCase()) {
